    ...
    return (
        <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800">
            ...
            <div className="flex flex-wrap gap-3 mb-4">
                {profiles.map(p => (
                    <Link href={`/profile/${p.id}`} key={p.id} title={p.first_name || 'User'}>
                        <Avatar seed={p.id} className="w-10 h-10 rounded-full border-2 border-transparent hover:border-indigo-500" />
                    </Link>
                ))}
            </div>
            <Link href={`/search?passion=${passionName}`} className="inline-flex items-center gap-2 text-sm text-indigo-400 hover:underline">
