...
const MultiSelect: React.FC<MultiSelectProps> = ({ options, selected, onChange, label, placeholder, id, name }) => {
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            ...
            <div className="flex flex-wrap gap-2 p-2 border border-gray-700 rounded-lg bg-gray-900 cursor-text hover:border-gray-600 transition-colors" onClick={() => setIsOpen(true)}>
                {selected.map((item) => (
                    ...
                    {filteredOptions.length > 0 ? (
                        filteredOptions.map((option) => (
                            <div
                                ...
                                className={`px-4 py-2 cursor-pointer hover:bg-gray-700 flex items-center justify-between ${selected.includes(option.name) ? 'bg-indigo-900/50 text-indigo-300' : 'text-gray-300'
                                    }`}
                                role="option"
                                aria-selected={selected.includes(option.name)}
                            >
                                <span>{option.name}</span>
                                {selected.includes(option.name) && <Check size={16} className="text-indigo-400" />}
                            </div>
                        ))
                    ) : (
