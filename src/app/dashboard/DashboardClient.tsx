...
export default function DashboardClient({ userId, initialProfile }: DashboardClientProps) {
        ...
        const fetchPanelsData = async () => {
            try {
                const [convosRes, suggestionsRes, passionsRes] = await Promise.all([
                    ...
                    (appClient as any).rpc('get_recent_conversations', { current_user_id: userId }),
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    (appClient as any).rpc('get_suggested_profiles', { current_user_id: userId, p_limit: 5 }),
                    appClient.from('profile_passions').select('passion_id, passions(name)').eq('profile_id', userId),
                ]);
    ...
    const welcomeName = getDisplayName(initialProfile.first_name);
    const introCopy = initialProfile.about_me || dashboardCopy.heroIntroFallback;
    const profilePassionsCount = initialProfile.passions_count ?? 0;
        ...
        <div className="editorial-shell py-8 sm:py-12 lg:py-16">
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 sm:mt-12 lg:grid-cols-3 sm:gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <ProfileCompletion profile={initialProfile} />
                ...
                <div className="space-y-8">
                    <div className="glass-panel rounded-[1.75rem] p-4 sm:p-6">
                        <PassionSpotlight userPassions={userPassions} userId={initialProfile.id} loading={panelsLoading} />
                    </div>
                </div>
            </div>
        </div>
