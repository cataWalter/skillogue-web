...
const isMissingOptionalEventCollectionError = (collection: string, error: unknown) => {
  if (!OPTIONAL_EVENT_COLLECTION_NAMES.has(collection)) {
    return false;
  }
  ...
  if (errorType === 'collection_not_found') {
    return true;
  }
  ...
  return lowerCase(getAppwriteErrorMessage(error, '')).includes('collection with the requested id');
};
...
export class AppDataService {
  ...
  private async listAllDocuments(collection: string, queries: string[] = []) {
    ...
    try {
      ...
      if (isMissingOptionalEventCollectionError(collection, error)) {
        return [];
      }
  ...
  private normalizeDateFilter(value?: string | null) {
    if (!value) {
      return null;
    }
  ...
  private async getEventDocument(eventId: string) {
    const documents = await this.listDocumentsPage('events', [Query.equal('id', eventId)], 1, 0);
    return documents[0] ?? null;
  }

  private canViewEvent(event: Record<string, any> | null | undefined, viewerId?: string | null) {
    if (!event) {
      return false;
    }

    if (viewerId && String(event.creator_id) === viewerId) {
      return true;
    }

    return EVENT_VISIBLE_STATUSES.has(String(event.status));
  }

  private async requireOwnedEvent(eventId: string, currentUserId: string) {
    const event = await this.getEventDocument(eventId);

    if (!event) {
      throw new Error('Event not found');
    }

    if (String(event.creator_id) !== currentUserId) {
      throw new Error('Only the event creator can modify this event');
    }

    return event;
  }

  private isEventWithinRange(event: Record<string, any>, startsFrom?: string | null, startsTo?: string | null) {
    const timestamp = Date.parse(String(event.starts_at ?? ''));
    ...
    if (startsFrom) {
      const startsFromTimestamp = Date.parse(startsFrom);

      if (!Number.isNaN(startsFromTimestamp) && timestamp < startsFromTimestamp) {
        return false;
      }
    }

    if (startsTo) {
      const startsToTimestamp = Date.parse(startsTo);

      if (!Number.isNaN(startsToTimestamp) && timestamp > startsToTimestamp) {
        return false;
      }
    }
  ...
  async getRsvpSummaryForEventIds(eventIds: Array<string | number>, currentUserId?: string | null) {
    const normalizedEventIds = unique(
      eventIds
    ...
    if (!normalizedEventIds.length) {
      return {} as Record<string, { attendance_count: number; is_attending: boolean }>;
    }

    let rsvpRows: Record<string, any>[] = [];

    try {
      rsvpRows = await this.listAllDocuments('event_rsvps', [Query.equal('event_id', normalizedEventIds)]);
    } catch {
      const allRows = await this.listAllDocuments('event_rsvps');
      rsvpRows = allRows.filter((row) => normalizedEventIds.includes(String(row.event_id)));
    }

    const summary: Record<string, { attendance_count: number; is_attending: boolean }> = {};
    ...
    for (const row of rsvpRows) {
      ...
      summary[eventId].attendance_count += 1;

      if (currentUserId && String(row.user_id) === currentUserId) {
        summary[eventId].is_attending = true;
      }
    }
  ...
  private async hydrateEvents(events: Record<string, any>[], viewerId?: string | null): Promise<EventSummary[]> {
    const creatorIds = unique(events.map((event) => event.creator_id).filter(Boolean));
    const locationIds = unique(events.map((event) => event.location_id).filter(Boolean));
    const creatorsById = await this.fetchByIds('profiles', creatorIds);
    const locationsById = await this.fetchByIds('locations', locationIds);
    const rsvpSummaryByEventId = await this.getRsvpSummaryForEventIds(
      events.map((event) => String(event.id)),
      viewerId
    );

    return events.map((event) => {
      const creator = creatorsById.get(String(event.creator_id));
      const location = locationsById.get(String(event.location_id));
      const rsvpSummary = rsvpSummaryByEventId[String(event.id)] ?? {
        attendance_count: 0,
        is_attending: false,
      };
      ...
      return {
        ...
        capacity: getNumberValue(event.capacity),
        status: String(event.status) as EventSummary['status'],
        ...
        location: location
          ...
          : null,
        attendance_count: rsvpSummary.attendance_count,
        is_attending: rsvpSummary.is_attending,
        is_owner: viewerId ? String(event.creator_id) === viewerId : false,
      };
    });
  }

  async createEvent(currentUserId: string, data: {
    title: string;
    description: string;
    location_id: string;
    starts_at: string;
    ...
    return this.getEventForViewer(eventId, currentUserId);
  }

  async updateEvent(currentUserId: string, data: {
    id: string;
    title: string;
    description: string;
    location_id: string;
    ...
    const existingEvent = await this.requireOwnedEvent(data.id, currentUserId);

    await this.repo.updateDocument(this.getCollectionId('events'), String(existingEvent._appwriteId ?? existingEvent.id), {
      title: data.title,
      description: data.description,
      location_id: data.location_id,
      starts_at: data.starts_at,
    ...
    return this.getEventForViewer(data.id, currentUserId);
  }

  async publishEvent(currentUserId: string, eventId: string) {
    const existingEvent = await this.requireOwnedEvent(eventId, currentUserId);
  ...
  async cancelEvent(currentUserId: string, eventId: string) {
    const existingEvent = await this.requireOwnedEvent(eventId, currentUserId);
  ...
  async listPublishedEvents(filters: EventSearchFilters = {}, viewerId?: string | null) {
    const normalizedQuery = lowerCase(filters.query);
    const startsFrom = this.normalizeDateFilter(filters.starts_from);
    const startsTo = this.normalizeDateFilter(filters.starts_to);
    const page = Math.max(1, Math.floor(getNumberValue(filters.page) ?? 1));
    const pageSize = Math.max(1, Math.min(50, Math.floor(getNumberValue(filters.page_size) ?? 12)));
    const allEvents = await this.listAllDocuments('events', [Query.orderAsc('starts_at')]);

    const filteredEvents = allEvents
      .filter((event) => String(event.status) === 'published')
      .filter((event) => !filters.location_id || String(event.location_id) === String(filters.location_id))
      .filter((event) => this.isEventWithinRange(event, startsFrom, startsTo))
      .filter((event) => {
        if (!normalizedQuery) {
          return true;
        }
      ...
      .sort((left, right) => compareValues(left.starts_at, right.starts_at));

    const total = filteredEvents.length;
    const offset = (page - 1) * pageSize;
    const events = await this.hydrateEvents(filteredEvents.slice(offset, offset + pageSize), viewerId);
  ...
  async listCreatorEvents(currentUserId: string) {
    const events = await this.listAllDocuments('events', [
      Query.equal('creator_id', currentUserId),
      Query.orderAsc('starts_at'),
    ]);

    return this.hydrateEvents(
      events.sort((left, right) => compareValues(left.starts_at, right.starts_at)),
      currentUserId
    );
  }

  async listCalendarEventsForUser(
    currentUserId: string,
    filters: {
      scope?: CalendarEventFilter;
      starts_from?: string;
      starts_to?: string;
    } = {}
  ) {
    ...
    const createdEvents = await this.listAllDocuments('events', [Query.equal('creator_id', currentUserId)]);
    const rsvpRows = await this.listAllDocuments('event_rsvps', [Query.equal('user_id', currentUserId)]);
    const attendingEventIds = unique(rsvpRows.map((row) => row.event_id).filter(Boolean));
    const attendingEvents = attendingEventIds.length
      ? await this.listAllDocuments('events', [Query.equal('id', attendingEventIds)])
      : [];
    const createdIds = new Set(createdEvents.map((event) => String(event.id)));
    const attendingIds = new Set(attendingEvents.map((event) => String(event.id)));
    const combinedEvents = new Map<string, Record<string, any>>();

    for (const event of [...createdEvents, ...attendingEvents]) {
      combinedEvents.set(String(event.id), event);
    }

    const filteredEvents = Array.from(combinedEvents.values())
      .filter((event) => this.canViewEvent(event, currentUserId))
      .filter((event) => {
        if (scope === 'created') {
          return createdIds.has(String(event.id));
        }

        if (scope === 'attending') {
          return attendingIds.has(String(event.id));
        }
    ...
    return this.hydrateEvents(filteredEvents, currentUserId);
  }

  async getEventForViewer(eventId: string, viewerId?: string | null) {
    const event = await this.getEventDocument(eventId);

    if (!this.canViewEvent(event, viewerId)) {
      return null;
    }

    const [hydratedEvent] = await this.hydrateEvents([event], viewerId);
    return hydratedEvent ?? null;
  }

  async setEventRsvp(currentUserId: string, eventId: string) {
    const event = await this.getEventDocument(eventId);
    ...
    if (String(event.status) !== 'published') {
      throw new Error('Only published events can accept RSVPs');
    }

    const existingRsvps = await this.listAllDocuments('event_rsvps', [
      Query.equal('event_id', eventId),
      Query.equal('user_id', currentUserId),
    ]);

    if (!existingRsvps.length) {
      const rsvpSummary = await this.getRsvpSummaryForEventIds([eventId], currentUserId);
      const attendanceCount = rsvpSummary[eventId]?.attendance_count ?? 0;
      const capacity = getNumberValue(event.capacity);

      if (capacity !== null && attendanceCount >= capacity) {
        throw new Error('This event is full');
      }

      const rsvpId = ID.unique();
      await this.repo.createDocument(
        this.getCollectionId('event_rsvps'),
        {
          id: rsvpId,
          event_id: eventId,
  ...
  async removeEventRsvp(currentUserId: string, eventId: string) {
    const existingRsvps = await this.listAllDocuments('event_rsvps', [
      Query.equal('event_id', eventId),
      Query.equal('user_id', currentUserId),
    ]);
    ...
    for (const rsvp of existingRsvps) {
      await this.repo.deleteDocument(
        this.getCollectionId('event_rsvps'),
        String(rsvp._appwriteId ?? rsvp.id)
      );
    }
