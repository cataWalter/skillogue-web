...
describe('ConversationItem', () => {
	it('renders a selected unread conversation, caps the badge, and handles clicks', () => {
		...
		expect(item).toHaveClass('bg-brand/10');
		expect(item).toHaveClass('border-l-brand');
	...
	it('renders fallback copy for conversations without unread messages', () => {
		...
		expect(item).not.toHaveClass('bg-brand/10');
		expect(container.querySelector('.bg-approval')).not.toBeInTheDocument();
	});
});
