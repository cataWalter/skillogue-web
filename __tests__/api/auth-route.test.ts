...
describe('/api/auth/[...all] route', () => {
	...
	const createRequest = (body?: unknown, userAgent: string | null = 'jest-test') =>
		({
			headers: {
	...
	it('omits the session name when Appwrite returns a null name and no user agent', async () => {
		...
		const response = await routeHandlers.GET(createRequest(undefined, null), routeParams('session'));
	...
	it('returns an email-verification payload without a name when the created user has no name', async () => {
	...
	it('changes the password for an authenticated session', async () => {
		const response = await routeHandlers.PUT(
			createRequest(
				{ oldPassword: 'Current123#', newPassword: 'NewPassword123#' },
				null
			),
			routeParams('change-password')
		);

		expect(getAppwriteSessionSecret).toHaveBeenCalled();
		expect(createAppwriteSessionAccount).toHaveBeenCalledWith('session-secret', undefined);
		expect(updatePassword).toHaveBeenCalledWith('NewPassword123#', 'Current123#');
		await expect(response.json()).resolves.toEqual({ success: true });
	});

	it('rejects change-password requests without both passwords', async () => {
		const response = await routeHandlers.PUT(
			createRequest({ oldPassword: null, newPassword: {} }),
			routeParams('change-password')
		);
	...
	it('maps change-password failures', async () => {
		const failure = Object.assign(new Error('Password update failed'), { code: 403 });
		updatePassword.mockRejectedValueOnce(failure);

		const response = await routeHandlers.PUT(
			createRequest({ oldPassword: 'Current123#', newPassword: 'NewPassword123#' }),
			routeParams('change-password')
		);

		expect(getAppwriteErrorMessage).toHaveBeenCalledWith(failure, 'Failed to change password.');
		expect(getAppwriteErrorStatus).toHaveBeenCalledWith(failure, 400);
		expect(response.status).toBe(403);
		await expect(response.json()).resolves.toEqual({ message: 'Password update failed' });
	});
